---
title: "ShinyFMBN v4_light Manual"
...
Thank you for downloading the ShinyFMBN app (or for using it on Shinyapps.io). The app was created using [R](https://cran.r-project.org/) and [Shiny](https://shiny.rstudio.com/) and is currently designed to run either locally on your machine or on [Shinyapps.io](https://ep142.shinyapps.io/ShinyFMBN_4_light/). The app will allow you to:
...
A few other options can be set in lines 29-61 of the app (the *Options* section).  
...
* the option of saving files for the conet app of Cytoscape has been removed: inference of association networks can be performed more efficiently using a phyloseq object with the [NetCoMi](https://github.com/stefpeschel/NetCoMi) package; however, I have added an option for downloading taxa, sample and OTU abundance tables as a .zip file  

* A few additional options for saving graphs can be set in the Save options tab.

* all objectcs which can be exported can now be saved using download buttons.  

* the app is now available on line as a [public beta](https://ep142.shinyapps.io/ShinyFMBN_4_light/); I hope I will find to finish testing soon 

# **Requisites**.

To run the app locally you need R version 4 or above (download from [CRAN](https://cran.r-project.org/)) and [RStudio](https://www.rstudio.com/products/rstudio/download/#download). Make sure that the software can be run with your system. Please note that the app will display large tables: you will be better off if you use a display which is 21.5 inches or larger. Using smaller displays is feasible but may require some horizontal and vertical scrolling.
...
To run the app on Shinyapps.io, just type this address in your browser: https://ep142.shinyapps.io/ShinyFMBN_4_light/
__Please note__: this version only allows 25 hours of active time/month and this is likely to limit the number of users as well. Be patient (I have to be, the administration of my University does not allow me to use my funding to buy a better plan).  

![Figure 1. Launching the app locally.](images/launchapp.png)
...
There are several ways to launch the app locally (for more info and options visit this page: [https://shiny.rstudio.com/articles/running.html](https://shiny.rstudio.com/articles/running.html)):
...
2. at the R console type^[otherwise use `runApp(ShinyFMBN_4, launch.browser = T)` to launch the app in your default browser or `runApp(ShinyFMBN_4;, launch.browser = T, display.mode = "showcase")` to do the same in showcase mode]
...
3. files will be saved in your download folder (when the app is used within RStudio) or in your default download folder (when run in a browser): see the Export tab for further details. 
...
**Known issues** sometimes the app my stall when trying complex filters; try checking <apply> again
...
Here you will be able to export your selection in a variety of formats and generate (and export) some further summary tables and graphs of the data after aggregation. You just need to provide a prefix for the file names and click on the action buttons. If running the app locally you will be given a choice to select a save folder, otherwise, in a browser, files will be saved to your default destination folder. Be careful not to overwrite the files.
...
1. **all** will save a list in .rds format including your selection of studies, samples, edges, taxa, and additional information on the references, version, sample Ids and sample and taxa aggregation.  

2. **agg** will save a list in .rds format including (after aggregation, if any), your selection of studies, an OTU table with absolute abundances, an OTU table with relative abundances, sample and taxa metadata, edges and node tables, an igraph object with the bipartite sample-taxa network, and additional information on the references, version, sample Ids and sample and taxa aggregation. In the future the list will be used by an interactive Shiny app for statistical and graphical analysis. This is also the file format you need for performing statistical and graphical analyses using the script [FMBNanalyzer](https://github.com/ep142/FoodMicrobionet/tree/master/FMBNanalyzer).    

3. **phy** will save a phyloseq class object in .Rdata format. The object is suitable for use with [Shiny-phyloseq](http://joey711.github.io/shiny-phyloseq/)^[and a number of Bioconductor packages, including phyloseq itself but also edgeR, DESeq2 and NetCoMi.] for statistical and graphical analysis.  

4. **zip** will save a compressed folder containing the "OTU" abundance matrix, the sample data and the taxonomic table in tab delimited format. You might want to change the name of some columns to reuse it with other applications.  
...
This tab features a box plot (the default, or a violin plot, selected using the plot type menu) of the relative abundance of the top most abundant 25 taxa. Choices made for sample aggregation in the bar plot tab will be reflected here, but you can use the sliders to choose the taxa aggregation level and the taxa to display^[by default all are selected; otherwise you can deselect all and add n taxa from the **select taxa** drop-down menu.]. Use the download button to save the graph (see above for details). **Please note**: reactives in this tab only become active after you have visualized the bar plot.  
...
It might have been better to add an option for rarefaction, and add some further personalization for the graphs, but I decided to leave this for later (which might as well be never).

If you want to use external links you are better off opening the app in the browser.

All text files are saved as tab-delimited files with "." as a delimiter; users in countries using "," as a delimiter must keep this into account when using or importing the files in their own locale.  


## **Testing**.

Testing of the app is in progress. As of January 2026 it has been successfully tested on the following systems:

* MacBook Pro 14 inch late 2023, Apple M3, 16 Gb RAM, MacOS Tahoe 26.1, R 4.5.2, RStudio 2026.01.0+392  for macOS 
...
This work was was carried out within the PRIN 2022 PNRR Project NCYdiversity P20229JMMH and received funding from the European Union Next-GenerationEU, CUP C53D23007560001 (PIANO NAZIONALE DI RIPRESA E RESILIENZA (PNRR) – MISSIONE 4 COMPONENTE 2,  INVESTIMENTO 1.4 – D.D. 1364/2023). This script and its contents reflects only the authors’ views and opinions,  neither the European Union nor the European Commission can be considered  responsible for them.