...
# in this version a download handler is used instead of ggsave(), save(), etc.
...
# change this between "locally", which will work best on your system, or "ShinyIO",
# which is designed to run on ShinyIO
use_app <- "locally" # or "ShinyIO"
...
if (use_app == "locally") {
  ...
  options(repos = BiocManager::repositories()) # needed on Shinyapps.io
...
app_text <- "ShinyFMBN (v4 light) is designed to provide a GUI to:"
...
ui <- navbarPage(
  ...
  tabPanel("Welcome",
           fluidPage(
             # welcome row  
             fluidRow(
               uiOutput("welcome_panel_text"),
               selectInput(
                 "select_version",
                 label = "Select a subset",
                 choices = c("Bacteria", "Fungi"),
                 selected = "Bacteria",
                 multiple = F
               )
             ),
           ...
           fluidPage(
             ...
             fluidRow(
               column(2,
                      ...
                      uiOutput('choose_L6'),
                      checkboxInput("L6_ch_box", label = "apply", value = F),
                      hr(),
                      strong("Instructions"),
                      p(
           ...
           sidebarLayout(
             ...
             sidebarPanel(
               ...
               p(
                 ...
                 "in all your filenames. Do not add extensions.",
                 style = "font-size:80%"
               ),
               textInput("fn_prefix", "Filename prefix", value = "myFMBN"),
               p(
                 "Use the download buttons to export files extracted from FoodMicrobionet in the output folder:",
                 style = "font-size:80%"
               ),
               downloadButton("download_mini", "all"),
               downloadButton("download_agg", "agg"),
               downloadButton("download_phy", "phy"),
               downloadButton("download_zip", "zip"),
               downloadButton("download_gml", "gml"),
               downloadButton("download_refs", "ref"),
               hr(),
               tags$div(
                 tags$ul(
                   tags$li(
                     tags$span(
                       "all: a .rds file with filtered FMBN tables, no aggregation"
                     )
                   ),
                   tags$li(
                     tags$span(
                       "agg: a .rds file with FMBN tables after aggregation, for further graphical and statistical analysis"
                     )
                   ),
                   tags$li(
                     tags$span(
                       "phy: a phyloseq class object, with OTU, sample and taxa tables, after aggregation, for use with Shiny-Phyloseq"
                     )
                   ),
                   tags$li(
                     tags$span(
                       "zip: a zip file with OTU, sample and taxa tables, after aggregation, as tab-delimited files"
                     )
                   ),
                   tags$li(
                     tags$span(
                       "gml: the bipartite sample and taxa network in .gml format, can be imported in Cytoscape and Gephi"
                     )
                   ),
                   tags$li(tags$span(
                     "ref: references for your selection (tab delimited, .txt)"
                   ))
                 ),
                 style = "font-size:80%"
               ),
             ...
             mainPanel(
               ...
               tabsetPanel(
                 ...
                 tabPanel(
                   ...
                   p(
                     "Use the download button to save the table in tab delimited format.",
                     style = "font-size:80%"
                   ),
                   hr(),
                   sliderInput(
                   ...
                   hr(),
                   downloadButton("download_otu", "'OTU' table"),
                   # diagnostic
                 ...
                 tabPanel(
                   ...
                   hr(),
                   downloadButton("download_prevabt", "prevab table"),
                 ),
                 ...
                 tabPanel(
                   ...
                   fluidRow(
                     ...
                     p(
                       "Use the download button to save the graph. ",
                       "The graph will be saved in the format and with the resolution selected in the 'Save options' tab (default 150 dpi, .jpg).",
                       style = "font-size:80%"
                     ),
                     hr()
                   ),
                   ...
                   fluidRow(
                     ...
                     hr(),
                     downloadButton("download_prevabg", "prevab graph"),
                     # diagnostic
                     if (debug_mode) {
                 ...
                 tabPanel(
                   ...
                   fluidRow(
                     ...
                     p(
                       "Use the download button at the bottom to save the graph. ",
                       "The graph will be saved in the format and with the resolution selected in the 'Save options' tab (default 150 dpi, .jpg).",
                       style = "font-size:80%"
                     ),
                     hr()
                   ...
                   fluidRow(
                     plotOutput("barplot"),
                     hr(),
                     downloadButton("download_barplot", "bar plot"),
                   ),
                 ...
                 tabPanel("Box plot",
                          fluidRow(
                            ...
                            p(
                              "Use the download button at the bottom to save the graph in the <tablesandplots> folder. ",
                              "The graph will be saved in the format and with the resolution selected in the 'Save options' tab (default 150 dpi, .jpg).",
                              style = "font-size:80%"
                            ),
                            hr()
                          ...
                          hr(),
                          downloadButton("download_boxplot", "boxplot"),
                          ),
...
tabPanel("Save options",
         fluidPage(
           fluidRow(
             p("Select the options for saving the graphs from the Export tab")
           ),
           fluidRow(
             column(3,
                    selectInput(
                      "select_format",
                      label = "Select graph format",
                      choices = c("jpeg", "tiff", "pdf"),
                      selected = "jpeg",
                      multiple = F
                    )
             ),
             column(3,
                    selectInput(
                      "select_res",
                      label = "Select graph resolution",
                      choices = c("96", "150", "300"),
                      selected = 150,
                      multiple = F
                    )
             ),
             column(3,
                    selectInput(
                      "select_width",
                      label = "Select graph width, in inches",
                      choices = c(5, 7, 9),
                      selected = 7,
                      multiple = F
                    )
             ...
             column(3,
                    selectInput(
                      "select_height",
                      label = "Select graph height, in inches",
                      choices = c(5, 7, 9),
                      selected = 5,
                      multiple = F
                    )
  ...
  tabPanel("About",
          ...
          sidebarLayout(
           sidebarPanel(
             uiOutput("about_panel_text"),
             uiOutput('copyright_text'),
             uiOutput("more_copyright_text")
            ),
            # end sidebar panel, About tab
           
            mainPanel(
             p(h4("The structure of FoodMicrobionet")),
             img(src = "FMBN_structure_5_0.jpg", height = 600)
            )
...
# reactively load data ----------------------------------------------------
    ...
    # reactively load the data
    FMBN_reactive <- reactive(
      {
        file_to_load <- if_else(input$select_version == "Fungi",
                                file.path("data","FMBN_F.rds"),
                                file.path("data","FMBN_B.rds"))
        FMBN <- readRDS(file_to_load)
        selected_study <- if_else(input$select_version == "Bacteria", "ST1", "ST161")
        # generate minitables for display
        studies_disp <- FMBN$studies %>%
          dplyr::select(
            studyId,
            FMBN_version,
            target,
        ...
        # the only change is that some variables become factors
        
        ssamples <- samples %>%
          mutate(across(.cols = c("s_type", "nature", "process", "spoilage", "target1", "target2"),
                        as.factor))
        ...
        target1_list <- unique(samples$target1)
        ...
        my_inputs <- list(
          ...
          studies_disp = studies_disp,
          s_type_list = s_type_list,
          nature_list = nature_list,
          process_list = process_list,
          spoilage_list = spoilage_list,
          target1_list = target1_list,
          target2_list = target2_list,
          ssamples = ssamples,
          app_text = app_text,
          copyright_text = FMBN$copyright_text
        )
        return(my_inputs)
      }
    ) %>%
      bindEvent(input$select_version)
    
    #the summary text and its UI element
    FMBN_summary_text <- reactive(
      {
        n_studies <- nrow(FMBN_reactive()$studies)
        n_samples <- nrow(FMBN_reactive()$samples)
        n_food_groups <- n_distinct(FMBN_reactive()$samples$L1)
        n_foodId <- n_distinct(FMBN_reactive()$samples$foodId)
        n_llabel <- n_distinct(FMBN_reactive()$samples$llabel)
        n_taxa <- nrow(FMBN_reactive()$taxa)
        group_text <- if_else(input$select_version == "Fungi", " fungal", "bacterial")
        FMBN_summary_text <-
          paste(
            FMBN_reactive()[[7]], 
            "There are",
            ...
            "studies and",
            n_samples,
            "samples in this version, which includes data on",
            group_text,
            ...
            n_taxa,
            "fungal and bacterial taxa have been found, with assignment performed at different taxonomic levels."
          )
      }
    ) %>%
      bindEvent(input$select_version)
    
    output$summary_text <- renderUI(FMBN_summary_text())
    
    output$app_text <- renderUI(FMBN_reactive()[[16]])
    
    output$copyright_text <- renderUI(FMBN_reactive()[[17]])
    
    # Welcome tab -------------------------------------------------------------
    
    output$welcome_panel_text <- renderUI({
      includeMarkdown(path = file.path("www", "welcome_panel.md"))
    })
    ...
    output$choose_study <- renderUI({
      study_names <- as.list(FMBN_reactive()[[2]]$studyId)
      selectInput(
        "view_study",
        label = h5("Select one or more studies to explore samples"),
        choices = study_names,
        selected = study_names[[1]],
        multiple = TRUE
      )
    })
    ...
    output$choose_study_filter <- renderUI(
      selectInput(
      "select_study",
      label = "Select a subset of studies",
      choices = FMBN_reactive()[[2]]$studyId,
      selected = NULL,
      multiple = T
    ))
    ...
    food_group_names <- reactive({
      nfstudy <- subset(FMBN_reactive()[[3]], studyId %chin% study_sel())
      unique(nfstudy$L1)
    })
    ...
    output$choose_food_group <- renderUI({
      selectInput(
        "select_L1",
        label = "Select one or more food groups (FoodEx2 L1)",
    ...
    # reactive, for the dynamic L6 list
    
    L6_names <- reactive({
      nfstudy <- subset(FMBN_reactive()[[3]], studyId %chin% study_sel())
      nffgroup <- subset(nfstudy, L1 %chin% food_group_sel())
      unique(nffgroup$L6)
    })
    
    # renders the L6 drop down
    output$choose_L6 <- renderUI({
      selectInput(
        "select_L6",
        label = "Select one or more foods (FoodEx2 L6)",
        choices = as.list(L6_names()),
        multiple = TRUE
      )
    })
    
    # creates the reactive which responds to select_L6
    L6_sel <- reactive({
      if (is.null(input$select_L6) | input$L6_ch_box == F) {
        L6_names()
      } else {
        input$select_L6
      }
    },
    label = "L6_sel_reactive")
    
    # creates the reactive to display the table
    f_fsamples <- reactive({
      fstudy <- subset(FMBN_reactive()[[15]], studyId %chin% study_sel())
      ffgroup <- subset(fstudy, L1 %chin% food_group_sel())
      fL6 <- subset(ffgroup, L6 %chin% L6_sel())
      ffinal <- fL6
    ...
    if (debug_mode) {
      ...
      sel_samples <- eventReactive(input$ssamplestable_rows_all, {
        samplesIds <- pull(f_fsamples()[input$ssamplestable_rows_all, 2])
        sel_samples_df <- dplyr::filter(FMBN_reactive()[[3]], sampleId %in% samplesIds) |>
          dplyr::select(studyId:label_1)
        # a subset from samples, these are sampleIds
        return(sel_samples_df)
      })
      output$agg_samples_mt <-
        DT::renderDataTable(sel_samples())
      # end diagnostic
    }
    ...
    FMBNfilt <- eventReactive(
      ...
      {
      if(any(c(input$study_ch_box, input$food_group_ch_box, input$L6_ch_box))){
      # extract samples based on row numbers of the filtered table
      ...
      sampleIds_input <- pull(f_fsamples()[input$ssamplestable_rows_all, 2])
      samples_filt <- FMBN_reactive()[[3]] %>%
         dplyr::filter(sampleId %in% sampleIds_input)  
      
      # get studies
      studies_filt <- FMBN_reactive()[[2]] %>%
        dplyr::filter(studyId %chin% pull(distinct(
          dplyr::select(samples_filt, studyId), studyId
        )))
      # get references
      reference_filt <- dplyr::bind_rows(FMBN_reactive()[[6]],
                                         dplyr::select(studies_filt,
                                                       studyId, ref_complete, DOI))
      # get edges
      edges_filt <-
        FMBN_reactive()[[4]] %>% dplyr::filter(sampleId %in% samples_filt$sampleId)
      ...
      # filter taxa
      taxa_filt <-
        FMBN_reactive()[[5]] %>% dplyr::filter(taxonId %in% taxonId_filt) %>%
        left_join(., taxon_ab)
      ...
      ),
      {
        if(any(c(input$study_ch_box, input$food_group_ch_box, input$L6_ch_box))) {
          ...
          edges_exp <- left_join(edges_exp,
                                 dplyr::select(FMBNfilt()[[2]], sampleId, label_2, llabel, foodId, L1, L6))
          
          # cast
          # NOTE FOR SELF: HERE AND ELSEWHERE, REPLACE WITH pivot_wider with values_fn
          # need to use tidyselect for names_from or use a combination of dplyr
          # for summarizing and pivot_wider for spreading
          OTUtableagg <-
            pivot_wider(
              data = edges_exp,
              id_cols = a_label,
              names_from = all_of(col_agg_var),
              values_from = weight,
              values_fill = 0,
              values_fn = sum
            )
    ...
    output$f_taxa_table <- DT::renderDataTable(
        ...
        FMBNexp()[[5]] %>%
          ...
          dplyr::mutate(
            ...
            Omni_habitat = case_when(
              ...
              TRUE ~ str_c("<a href='",
                           "https://omnicrobe.migale.inrae.fr/searchByTaxon?taxon=", 
                           label, 
                           "' target='_blank'>",
                           "go!",
                           "</a>",
            ...
            Omni_pheno = case_when(
              ...
              idelevel %in% c("domain", "phylum", "class", "order") ~ "no search path",
              TRUE ~ str_c("<a href='",
                           "https://omnicrobe.migale.inrae.fr/searchByTaxonForPhenotype?taxon=", 
                           label, 
                           "' target='_blank'>",
                           "go!",
                           "</a>",
            ...
            Omni_use =case_when(
              ...
              idelevel %in% c("domain", "phylum", "class", "order") ~ "no search path",
              TRUE ~ str_c("<a href='",
                           "https://omnicrobe.migale.inrae.fr/searchByTaxonForUse?taxon=", 
                           label, 
                           "' target='_blank'>",
                           "go!",
                           "</a>",
      ...
      colnames = c(
        ...
        "species" = "species",
        "NCBI" = "NCBI_outlink",
        "Other_tax_db" = "Secondary_outlink",
        "Om.habitat" = "Omni_habitat",
        "Om.pheno" = "Omni_pheno",
        "Om.use" = "Omni_use"
      ),
      options = list(
    ...
    # save selection before aggregation as .rds
    output$download_mini <- downloadHandler(
      filename = function() {
        paste0(input$fn_prefix, "_all.rds")
      },
      content = function(file) {
        req(FMBNfilt())
        saveRDS(object = FMBNfilt(), file = file)
        showNotification(
          "saving FMBN file...",
          type = "message",
          duration = 10,
    ...
    # save selection after aggregation as .rds
    output$download_agg <- downloadHandler(
      filename = function() {
        paste0(input$fn_prefix, "_agg.rds")
      },
      content = function(file) {
        req(FMBNexp())
        saveRDS(object = FMBNexp(), file = file)
        showNotification(
          "saving aggFMBN file",
          type = "message",
          duration = 10,
    ...
    # create and save a phyloseq object after aggregation
    output$download_phy <- downloadHandler(
      filename = function() {
        paste0(input$fn_prefix, "_physeq.Rdata")
      },
      content = function(file) {
        req(FMBNexp())
        showNotification(
          "assembling the phyloseq",
          type = "message",
          duration = 10,
          closeButton = TRUE
        )
        myphyseq <- (function(phydata){
          taxtable <- phydata[[5]] |> dplyr::select(label, domain:species)
          ...
          physeq <-
            phyloseq(otu_table(t(phydata[[2]]), taxa_are_rows = T),
                     tax_table(taxtable),
                     sample_data(phydata[[4]]))
          return(physeq)
        })(phydata = FMBNexp())
        save(myphyseq, file = file)
        showNotification(
          "saving your data as a phyloseq...",
          type = "message",
          duration = 10,
          closeButton = TRUE
        )
    ...
    # create and save save a zip file with tables after aggregation
    output$download_zip <- downloadHandler(
      filename = function() {
        paste0(input$fn_prefix, "_tables.zip")
      },
      content = function(file) {
        req(FMBNexp())
        showNotification(
          "assembling the .zip file with the tables...",
          type = "message",
          duration = 10,
          closeButton = TRUE
        )
        ...
        taxtable <- FMBNexp()[[5]] |> 
          dplyr::select(OTU = label, domain:species)
        
        otutable <- t(FMBNexp()[[2]]) |> 
          as.data.frame() |>
          rownames_to_column("OTU")
        sampletable <- FMBNexp()[[4]] |>
          select(NAME = label, studyId, llabel:SRA_run)
        
        # Define file paths in temp directory
        file1 <- file.path(temp_dir, "tax_table.txt")
        file2 <- file.path(temp_dir, "otutable.txt")
        file3 <- file.path(temp_dir, "sampletable.txt")
        
        # Write the files as tab-delimited
        write.table(taxtable, file1, sep = "\t", row.names = FALSE, dec = ".", na = "")
        write.table(otutable, file2, sep = "\t", row.names = FALSE, dec = ".")
        write.table(sampletable, file3, sep = "\t", row.names = FALSE, dec = ".")
        ...
        showNotification(
          "saving your data as a zip file...",
          type = "message",
          duration = 10,
          closeButton = TRUE
        )
    ...
    # download_gml
    output$download_gml <- downloadHandler(
      filename = function() {
        paste0(input$fn_prefix, ".gml")
      },
      content = function(file) {
        req(FMBNexp())
        write_graph(graph = FMBNexp()[[8]], file = file,
                    format = "gml")
        showNotification(
          "saving your data as a graph in gml format...",
          type = "message",
          duration = 10,
          closeButton = TRUE
        )
    ...
    output$download_refs <- downloadHandler(
      filename = function() {
        paste0(input$fn_prefix, "_refs.txt")
      },
      content = function(file) {
        req(FMBNfilt())
        write_tsv(FMBNfilt()[[5]], file)
        showNotification(
    ...
    my_summaries <- eventReactive(
      ...
      ),
      {
        if (!input$show_summary){
          ...
          if(input$select_version == "Bacteria"){
            # remove Eukaryota, Fungi, Chloroplast, Mitochondria, Other
            euk_labels <- FMBNexp()[[5]] %>%
              dplyr::filter(domain == "Eukaryota" | domain == "Fungi" | family == "Mitochondria" | class == "Chloroplast") %>%
              pull(label)
            remove_what <- c(euk_labels, "Other", "Chloroplast", "Mitochondria")
            to_remove <- which(colnames(my_OTU_table) %in% remove_what)
            if(length(to_remove)>0){
              my_OTU_table <-  my_OTU_table[,-to_remove]
            }
          }
          ...
          if(input$select_version == "Bacteria"){
            ftoptaxa <- ftoptaxa %>%
              dplyr::filter(!(label %chin% remove_what))
          }
    ...
    output$download_otu <- downloadHandler(
      filename = function() {
        paste0(input$fn_prefix, "_filt_otu.txt")
      },
      content = function(file) {
        req(filt_OTU_table())
        write_tsv(filt_OTU_table(), file)
        showNotification(
    ...
    output$download_prevabt <- downloadHandler(
      filename = function() {
        paste0(input$fn_prefix, "_prevab.txt")
      },
      content = function(file) {
        write_tsv(my_summaries()[[5]], file)
        showNotification(
          "saving your prevalence and abundance table...",
          type = "message",
          duration = 10,
          closeButton = TRUE
        )
    ...
    output$download_prevabg <- downloadHandler(
      filename = function() {
        paste0(input$fn_prefix, 
               "_prev_ab_plot.",
               input$select_format)
      },
      content = function(file) {
        ...
        ggsave(filename = file,
               plot = prev_ab_plot(),
               width = as.numeric(input$select_width),
               height = as.numeric(input$select_height),
               units = g_units,
               dpi = as.numeric(input$select_res))
        showNotification(
          "saving your prevalence and abundance graph...",
          type = "message",
          duration = 10,
          closeButton = TRUE
        )
    ...
    bb_plot_data <- eventReactive(
      ...
      ),
      {
        ...
        edge_table_filtered <- t(OTU_f_for_barplot) |>
          as.data.frame() |>
          rownames_to_column(var = "source") |>
          pivot_longer(
            cols = 2:(dim(OTU_f_for_barplot)[1]+1),
            names_to = "target",
            values_to = "weight"
          ) |>
          dplyr::filter(weight>0)
        ...
        bb_plot_list_1[[1]] <- bxp_edge_table_r_j # bxp_edge_table_r_j
    ...
    # the download button for saving the bar plot
    ...
    output$taxa_agg_bxp <- renderUI({
      selectInput(
        ...
        selected = if_else(length(as.list(bb_plot_data_2()[[4]][-1]))>=2,"2","1"),
        multiple = FALSE
      )
    })
    ...
    # the code for saving the box plot
    ...
    output$about_panel_text <- renderUI({
      includeMarkdown(path = file.path("www","about_panel.md"))
    })
    output$more_copyright_text <- renderUI({
      includeMarkdown(path = file.path("www","about_panel_copyright.md"))
    }) 
