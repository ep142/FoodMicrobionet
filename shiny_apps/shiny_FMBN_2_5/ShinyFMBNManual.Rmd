...
**This version allows access to data for Bacteria and Fungi**: the default is set to bacteria but, if you want to access Fungi, go to line 89 of the app and set `version_to_load <- "F"`.  
...
* (2_4_1) removed the button for export for MicrobiomeAnalyst, because of problems of compatibility. use the phyloseq object with [ShinyPhyloseq](https://joey711.github.io/shiny-phyloseq/) instead.

* (2.5)  there is an early option for choosing which list must be loaded, that for bacteria or fungi (set this in the version_to_load variable of the app); sample extraction now relies on `%chin%` and `%in%`
...
- provided links to the Omnicrobe (formerly Florilege) database
...
* MacBook Pro 14\'' 2021, M1 pro 8 core, 16 Gb RAM, MacOS Monterey v12.4, R 4.2.0 "Vigorous Calisthenics"


However, changes over previous versions (2.1.3, 2.2, 2.3, 2.4) are minimal and I cannot see why it should not work on other systems. Apart of this, I have not had any major bug reports, so people seem reasonably happy using it.  
