...
The aggregate tab will display all the relevant tables from FoodMicrobionet after the application of your filters^[if you do not use any filter and tick at least the <apply> checkbox for selecting studies, all studies, samples and taxa will be shown but, because of the size of the matrices (>3x10^6^ elements) that are created, the app may freeze for several minutes.].
...
4. **conet** will save a OTU table (absolute abundance) and a sample metadata table (both after aggregation, if any), ready for use with the [Conet](http://psbweb05.psb.ugent.be/conet/) app of [Cytoscape](http://cytoscape.org/).  
...
6. **mba** _ **will save OTU, sample metadata and taxonomy tables in a format suitable for use with*[MicrobiomeAnalyst](https://www.microbiomeanalyst.ca/upload/OtuUploadView.xhtml)^[When uploading the files in MicrobiomeAnalyst remember to choose Not specific /Other for Taxonomy labels.]  

7. **ref** will save the table for the references (in case you don&#39;t want to save a or b above)  
...
The **Aggregate** tab provides a few quick graphs and tables, which can be accessed using the tabs in the right side of the window.  
...
However, changes over previous version (2.1.3) are minimal and I cannot see why it should not work on other systems. Apart of this, I have not had any major bug reports, so people seem reasonably happy using it.  
