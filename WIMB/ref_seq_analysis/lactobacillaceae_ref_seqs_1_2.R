#########1#########2#########3#########4#########5#########6#########7#########8
# lactobacillaceae_ref_seqs v1.1 16/12/2022
#########1#########2#########3#########4#########5#########6#########7#########8
...
#      Lactobacillaceae_metadata_w_outg.txt (it includes an outgroup column)
...
for(i in seq_along(ts_seq_list)){
  ...
  if(length(dupl_seqs)>0) {seqs <- seqs[-which(duplicated(seqs))]}
...
conf_matrix_table <- conf_matrices %>%
  dplyr::select(region:estimate) %>%
  pivot_wider(names_from = term, values_from = estimate)

conf_matrix_table %>% 
  dplyr::select(region, class, sensitivity:neg_pred_value) %>%
  write_tsv(file = "conf_matrix_table.txt")
...
ref_seq_list <- list(
  full_length = c(ts_seq_list$FL, ref_seq_fl),
  V1V3 = c(ts_seq_list$V1V3, ref_seq_V1V3),
  V3V4 = c(ts_seq_list$V3V4, ref_seq_V3V4),
  V4 = c(ts_seq_list$V1V3, ref_seq_V4)
)
...
type_strains_seq_df <- select(ts_ides_ann$FL, seq.text = mod_seq, seq_abbr = seq.name, Genus, Species)
...
# this is supplementary figure 4 in the manuscript
